...
function addToDOM(transaction) {
    ...
    item.innerHTML = `
    <span class="transaction-text">${transaction.text} <strong>${sign}Rs${Math.abs(transaction.amount)}</strong></span>
...
function updateValues() {
    ...
    const expense = amounts.filter(v => v < 0).reduce((a, v) => a + v, 0).toFixed(2);
